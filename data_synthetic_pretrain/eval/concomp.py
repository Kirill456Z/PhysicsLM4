"""ConComp evaluation implementation."""
...
def _tok_repr(token_id: int, base_vocab_size: int) -> str:
    B = base_vocab_size
    if token_id == 0:
        return "<pad>"
    if 1 <= token_id <= B:
...
def _seq_repr(tokens: List[int], base_vocab_size: int, max_tokens: int = 40) -> str:
    parts = [_tok_repr(t, base_vocab_size) for t in tokens]
...
def _decode_answer(tokens: List[int], base_vocab_size: int) -> List[Tuple[int, ...]]:
    words = split_into_words(tokens, base_vocab_size)
...
class ConCompEvaluator:
    ...
    def evaluate_example(self, example: Dict) -> Dict:
        ...
        pred_vertices = _decode_answer(predicted_raw, self.base_vocab_size)
        exp_vertices = _decode_answer(expected_tokens, self.base_vocab_size)
        pred_set = set(pred_vertices)
        exp_set = set(exp_vertices)
        return {
            "skipped": False,
            "node_set_accuracy": pred_set == exp_set,
            "node_count_match": len(pred_vertices) == len(exp_vertices),
            "exact_seq_match": predicted_raw == expected_tokens,
            "pred_len": len(pred_vertices),
            "expected_len": len(exp_vertices),
            "_prompt": prompt_tokens,
            "_predicted_raw": predicted_raw,
            "_expected_tokens": expected_tokens,
        }

    def _log_examples(self, results: List[Dict], answer_size: int) -> None:
        logged = 0
        logger.info("\n%s", "=" * 70)
        logger.info("  SAMPLE EXAMPLES  size=%s  (first %s)", answer_size, _NUM_LOG_EXAMPLES)
        logger.info("%s", "=" * 70)
        for i, res in enumerate(results):
            if res.get("skipped") or logged >= _NUM_LOG_EXAMPLES:
                continue
            logged += 1
            correct = "CORRECT" if res["node_set_accuracy"] else "WRONG"
            prompt = _seq_repr(res["_prompt"], self.base_vocab_size)
            pred = _seq_repr(res["_predicted_raw"], self.base_vocab_size)
            exp = _seq_repr(res["_expected_tokens"], self.base_vocab_size)
            logger.info(
                "  [%s] %s\n    context (tail): %s\n    expected      : %s  (%s nodes)\n    predicted     : %s  (%s nodes)",
                i,
                correct,
    ...
    def evaluate_size(self, eval_file: str, answer_size: int, max_examples: Optional[int] = None) -> Dict:
        ...
        self._log_examples(all_results, answer_size)
        denom = max(n_eval, 1)
...
def evaluate_concomp(cfg: ConCompEvalArgs):
    ...
    eval_files = sorted(Path(cfg.eval_dir).glob("concomp.eval.size_*.jsonl"))
    if not eval_files:
    ...
    for eval_file in eval_files:
        ...
        all_results[f"size_{size}"] = evaluator.evaluate_size(str(eval_file), answer_size=size, max_examples=cfg.max_examples)
    out_file = Path(cfg.dump_dir) / "concomp_results.json"
    with open(out_file, "w") as f:
        json.dump(all_results, f, indent=2)
    if wandb.run is not None:
        metrics = {}
        for key, res in all_results.items():
            metrics[f"eval/concomp/{key}/node_set_accuracy"] = res["node_set_accuracy"]
            metrics[f"eval/concomp/{key}/node_count_match"] = res["node_count_match"]
            metrics[f"eval/concomp/{key}/exact_seq_match"] = res["exact_seq_match"]
            metrics[f"eval/concomp/{key}/length_ratio"] = res["length_ratio"]
        avg_set = sum(r["node_set_accuracy"] for r in all_results.values()) / max(len(all_results), 1)
        metrics["eval/concomp/avg_node_set_accuracy"] = avg_set
...
__all__ = ["ConCompEvalArgs", "evaluate_concomp"]
