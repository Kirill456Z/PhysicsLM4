...
\begin{frame}{The Synthetic Playground}
  \begin{columns}[T]
    \column{0.55\textwidth}
      ...
      \vspace{0.4em}
      \small The bet: a component that helps on these tasks is worth carrying to scale.
...
\begin{frame}{Canon: Motivation to Re-examine}
  ...
  \begin{itemize}
    \item Can a single training run on these benchmarks be trusted to rank
          two architectures?
    \item Evluated correctly, is Canon better. What does it
          change in training dynamics?
  \end{itemize}

  \vspace{0.8em}
  $\Rightarrow$ Two questions: measurement reliability first, then mechanism.
\end{frame}
...
\begin{frame}{Experimental Setup}
  \begin{columns}[T]
    \column{0.5\textwidth}
      ...
      \textbf{Canon:} CUDA kernel from Dao-AILab/causal-conv1d

      \vspace{0.4em}
      \textbf{Models:} Canon-ABCD vs.\ matched Llama (RoPE), identical width,
      depth, optimiser, data, token budget.
...
\begin{frame}{High variance requires more runs; Canon beats Llama}
  \begin{columns}[T]
    ...
    \column{0.43\textwidth}
      \textbf{Final loss vs.\ parameters} (5 seeds averaged per point).
      \begin{itemize}
